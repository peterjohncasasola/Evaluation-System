      ...
      <form @submit.prevent="save()" novalidate>
       <div class="modal-card">
        <header class="modal-card-head">
         <h3 class="modal-card-title">{{ isNew ? "Creating" : "Editing" }}</h3>
        ...
        <section class="modal-card-body">
         <b-field>
          ...
          <b-autocomplete
           ...
           @select="
            (option) =>
             (this.formData.course_id = option === null ? '' : option.id)
           "
           :clearable="true"
          >
         ...
         <b-field label="Academic Year">
          ...
          <b-autocomplete
           ...
           field="description"
           @select="
            (option) => (this.formData.sy_id = option === null ? '' : option.id)
           "
           :clearable="true"
          ></b-autocomplete>
         </b-field>
     ...
     >
      <template slot-scope="props">
       ...
       <b-table-column
        ...
        >{{
         props.row.prerequisite == null ? "None" : props.row.prerequisite
        }}</b-table-column
       >
       ...
       <b-table-column label="Semester" field="semester" width="10%" sortable>{{
        props.row.semester
       }}</b-table-column>
  ...
  getFilteredSubjectTags(text) {
   this.filteredSubjectsTags = this.subjects.filter((opt) => {
    return (
     ...
     opt.description.toString().toLowerCase().indexOf(text.toLowerCase()) >= 0
    );
   });
  },
