<template>
 <div>
  ...
  <section class="section is-main-section">
   <card-component class="has-table has-mobile-sort-spaced" title="Semesters">
    <b-table :hoverable="true" :data="semesters">
     <template slot-scope="props">
      <b-table-column searchable label="Semesters" field="semester" sortable>{{
       props.row.semester
      }}</b-table-column>
      ...
      <b-table-column searchable label="Status" field="is_active" sortable>
       <b-switch
        @input="setSemester(props.row)"
        :value="props.row.is_active == 1 ? true : false"
        type="is-success"
       >
       </b-switch>
      </b-table-column>
...
export default {
 ...
 methods: {
  setSemester(sem) {
   ...
   apiClient.get(`/settings/semesters/set/${sem.id}`);
   this.getSemesters();
  },
